...
(function () {
   ...
   */
  function newuploadHttpConcurrentProgress(url, type, concurrentRuns, timeout, testLength, callbackComplete, callbackProgress,
   ...
   * @return error object
   */
  newuploadHttpConcurrentProgress.prototype.onTestTimeout = function (error) {
    if (this._running) {
   ...
   * @return error object
   */
  newuploadHttpConcurrentProgress.prototype.onTestAbort = function () {
    if (this._running) {
   ...
   * @return error object
   */
  newuploadHttpConcurrentProgress.prototype.onTestError = function (error) {
    if (this._running) {
      this.clientCallbackError(error);
      this._running = false;
    }
   ...
   * onComplete method
   */
  newuploadHttpConcurrentProgress.prototype.onTestComplete = function (result) {
    if (!this._running) {
      return;
    }
    this._collectMovingAverages = false;
   ...
   */
  newuploadHttpConcurrentProgress.prototype.calculateStats = function () {
      ...
      if (lastElem > 0) {
        ...
        for (var j = 1; j <= lastElem; j++) {
          if (isFinite(this._progressResults[arrayData][this._progressResults[arrayData].length - j])) {
            ...
            console.log(singleMovingAverage);
          }
        }

        var reportingArray = this._progressResults[arrayData].slice(this._progressResults[arrayData].length-lastElem,this._progressResults[arrayData].length-1);
   ...
   * Start the test
   */
  newuploadHttpConcurrentProgress.prototype.start = function () {
    var request;
    ...
    if (this.type === 'GET') {
      ...
      for (var p = 1; p <= this.concurrentRuns; p++) {
        ...
        this['arrayResults' + this._testIndex] = [];
        this._progressResults['arrayProgressResults' + this._testIndex] = [];
        request = new window.newxmlHttpRequest('POST', this.url, this.timeout, this.onTestComplete.bind(this), this.onTestProgress.bind(this),
          this.onTestAbort.bind(this), this.onTestTimeout.bind(this), this.onTestError.bind(this));
        this._activeTests.push({
          xhr: request,
          testRun: this._testIndex
   ...
   * Cancel the test
   */
  newuploadHttpConcurrentProgress.prototype.abortAll = function () {
    this._running = false;
    for (var i = 0; i < this._activeTests.length; i++) {
      if (typeof(this._activeTests[i]) !== 'undefined') {
        this._activeTests[i].xhr._request.abort();
   ...
   * init test suite
   */
  newuploadHttpConcurrentProgress.prototype.initiateTest = function () {
    this._testIndex = 0;
  ...
  window.newuploadHttpConcurrentProgress = newuploadHttpConcurrentProgress;
})();
